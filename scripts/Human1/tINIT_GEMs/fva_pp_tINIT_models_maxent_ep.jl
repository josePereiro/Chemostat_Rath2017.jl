...
# +
zeroth = 1e-5
for fva_pp_file in fva_pp_files
    
    println("\n\n --------------- $(basename(fva_pp_file)) ---------------\n\n")
    dat = deserialize(fva_pp_files[1]);
    model = dat.metnet;
    obj_ider = params["obj_ider"]
    println("Before: ")
    fbaout = Ch.LP.fba(model, obj_ider)
    ...
    model.lb .= [abs(lb) < zeroth ? 0.0 : lb for lb in model.lb]
    model.ub .= [abs(ub) < zeroth ? 0.0 : ub for ub in model.ub];
    println("\tblocked: ", length(Ch.Utils.blocks(model)))
    model = Ch.Utils.del_blocked(model);
    ...
    serialize(fva_pp_file, (id = dat.id, metnet = model, src = dat.src))
