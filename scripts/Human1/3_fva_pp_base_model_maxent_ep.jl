...
@everywhere temp_cache_file(state...) = 
    joinpath(H1.MODEL_CACHE_DATA_DIR, "$(notebook_name)___temp_cache___state_$(hash(state)).jls")
...
@everywhere function process_stst(stst, upfrec = 10)
    ...
    βs = range(10.0^2, 10.0^5.7, length = 50) |> collect |> unique |> sort |> reverse
...
@everywhere function process_xi(stst, ξi, ξs, βs, upfrec)
    ...
    # If cached load 
    xi_tcache_file = temp_cache_file(stst, ξ)
...
## Saving
file_ = joinpath(H1.MODEL_CACHE_DATA_DIR, "$(notebook_name)___boundles.jls")
