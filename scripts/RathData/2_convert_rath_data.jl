...
stdm_orig = CSV.read(Rd._stand_medium_raw_file(), DataFrame; delim = "\t");
...
for met in ["GLU", "ALA", "ARG", "ASN", "ASP", "CYS", "GLY", 
    ...
    local idx = findfirst(stdm_conv.id .== met)
    stdm_conv[idx, [:conc, :unit]] .= [stdm_orig[idx, :conc] * 1e-3, "mM"]
end
# -

# Saving
CSV.write(Rd._stand_medium_proc_file(), stdm_conv, delim = "\t")
println("created $(relpath(Rd._stand_medium_proc_file()))!!!")
...
for exp in Rd.EXPS
    ...
    orig_file_path = Rd._cont_culture_raw_file(exp)
    ...
    conv_data = DataFrame(id = String[], val = Float64[], err = Float64[], unit = String[])
    ...
    conv_file_path = Rd._cont_culture_proc_file(exp)
...
max_flux_data_orig = CSV.read(Rd._max_invitro_fluxs_raw_file(), DataFrame; delim = "\t")
...
CSV.write(Rd._max_invitro_fluxs_proc_file(), max_flux_data_conv, delim = "\t")
println("created $(relpath(Rd._max_invitro_fluxs_proc_file()))!!!")
