function collect_protless(model::ChU.MetNet, idxs = eachindex(model.rxns))
    filter(idxs) do rxni
...
function prot_stois(ref_model)
    ...
    prot_pool_idx = ChU.metindex(ref_model, PROT_POOL)
    foreach(eachindex(ref_model.mets)) do meti
        ...
        rxnis = ChU.met_rxns(ref_model, meti)
        for rxni in rxnis
            rxn = ref_model.rxns[rxni]
            if startswith(rxn, DRAW_PREFFIX)
                s = ChU.S(ref_model, prot_pool_idx, rxni)
                push!(prot_draw_stois, s) 
            else
                s = ChU.S(ref_model, meti, rxni)
...
function merge_protless!(model, allowed_protless, prot_kin_stoi, prot_draw_stoi)
    ...
    disallowed_protless = filter(setdiff(all_protless, allowed_protless)) do rxn
        rxn != EMPTY_SPOT && !ChU.is_exchange(model, rxn)
    end |> unique!
    ...
    for (i, fwd_rxn) in disallowed_protless |> enumerate
        ...
        fwd_rxni = ChU.rxnindex(model, fwd_rxn) # forward defined
        rev = ChU.isrev(model, fwd_rxni)
        
        ## forward reaction
        # A + (-prot_kin_stoi) prot_met -> B
        # add prot_met
        prot_met = "prot_" * fwd_rxn # This must make it unique
        prot_meti = ChU.find_free_spot(model, :mets)
        ...
        draw_rxni = ChU.find_free_spot(model, :rxns)
        ...
        prot_pooli = ChU.metindex(model, Human1.PROT_POOL)
        ...
        if rev
            ...
            bkwd_rxni = ChU.find_free_spot(model, :rxns)
